...
class AddressBook(UserDict):
    ...
    def delete(self, name):
        if name in self.data.keys():
            ...
            View_message("Can't find record", 'red').output()
...
class Phone(Field):
    ...
    def value(self, phone):
       if re.search('\+\d{12}', phone) != None:
          ...
          View_message("Phone should have format: '[+] [XX] XXXXXXXXXX' (9 or 12 digits)", 'red').output()  
...
class Email(Field):
    ...
    def value(self, email):
       if re.search('[a-zA-Z0-9\.\-\_]+@[a-zA-Z0-9\-\_\.]+\.[a-z]{2,4}', email) != None:
          ...
          View_message("Email should have format: 'name@domain.[domains.]high_level_domain'", 'red').output()  
